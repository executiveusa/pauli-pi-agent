...
const migrationFile = join(__dirname, "../../ops/migrations/001_init_schema.sql");
...
describe("Database Schema", () => {
	...
	test("sources table has required columns", () => {
		const sql = readFileSync(migrationFile, "utf-8");
		const sourcesTableMatch = sql.match(/CREATE TABLE IF NOT EXISTS sources \([\s\S]*?\);/);
	...
	test("personas table has required fields", () => {
		const sql = readFileSync(migrationFile, "utf-8");
		const personasTableMatch = sql.match(/CREATE TABLE IF NOT EXISTS personas \([\s\S]*?\);/);
	...
	test("reasoning_runs table references personas", () => {
		const sql = readFileSync(migrationFile, "utf-8");
		const table = sql.match(/CREATE TABLE IF NOT EXISTS reasoning_runs \([\s\S]*?\);/);
	...
	test("audit_events table has required columns", () => {
		const sql = readFileSync(migrationFile, "utf-8");
		const table = sql.match(/CREATE TABLE IF NOT EXISTS audit_events \([\s\S]*?\);/);
		expect(table).toBeDefined();
	...
	test("secrets_references never stores raw secrets", () => {
		const sql = readFileSync(migrationFile, "utf-8");
		const table = sql.match(/CREATE TABLE IF NOT EXISTS secrets_references \([\s\S]*?\);/);
		expect(table).toBeDefined();
...
describe("Schema Version Management", () => {
	...
	test("schema file is idempotent (uses IF NOT EXISTS)", () => {
		...
		const ifNotExistsCount = (sql.match(/CREATE [A-Z]+ IF NOT EXISTS/g) || []).length;
