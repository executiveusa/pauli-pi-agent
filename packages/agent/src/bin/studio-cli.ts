...
import { PostizScheduler } from "../../../integrations/postiz/postiz.scheduler.js";
import type { PostizPost } from "../../../integrations/postiz/postiz.types.js";
import { OpusClipService } from "../video/opus-clip.service.js";
		...
		switch (command) {
			...
			case "status": {
				...
				if (result.clips && result.clips.length > 0) {
					result.clips.forEach(
						(clip: { title: string; downloadUrl: string; score: number; description: string }, index: number) => {
			...
			case "publish": {
				...
				const clipNum = parseInt(args[2], 10);

				if (!targetId || Number.isNaN(clipNum)) {
