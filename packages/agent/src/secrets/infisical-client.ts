...
export class InfisicalClient {
	 ...
	 * 4. Not found
	 */
	async resolveSecret(secretName: string, infisicalPath?: string): Promise<SecretResolution> {
	 ...
	 */
	async getSecret(secretName: string, infisicalPath?: string, required = true): Promise<string> {
	...
	async getSecrets(
		...
		for (const secret of secrets) {
			try {
				result[secret.name] = await this.getSecret(secret.name, secret.path, secret.required !== false);
			} catch (error) {
	...
	getStatus(): {
		...
		return {
			...
			infisicalConfigured: !!(this.config.clientId && this.config.clientSecret && this.config.projectId),
...
export async function resolveSecret(name: string, path?: string): Promise<SecretResolution> {
	const client = getSecretsClient();
	await client.initialize();
	return client.resolveSecret(name, path);
}

export async function getSecret(name: string, path?: string, required?: boolean): Promise<string> {
	const client = getSecretsClient();
	await client.initialize();
