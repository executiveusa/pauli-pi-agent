...
const SECRET_PATTERNS: Record<string, RegExp> = {
	// OpenAI/OpenRouter format: sk-* or sk-proj-* followed by characters
	openaiKey: /sk-(?:proj-)?[a-zA-Z0-9_-]{6,}/g,
	// Anthropic format: sk-ant-* or claude-*
	anthropicKey: /(sk-ant-|claude-)[a-zA-Z0-9_-]{6,}/g,
	...
	apiKey: /api[_-]?key\s*[=:]\s*['"]?([a-zA-Z0-9_\-.]{6,})['"]?/gi,
	// Database URLs
	...
	// Firecrawl API key
	firecrawlKey: /fc-[a-zA-Z0-9_-]{6,}/g,
	// BrightData/Proxy API key
	brightDataKey: /brd-[a-zA-Z0-9_-]{6,}/g,
};
...
export class SecretRedactor {
	...
	containsSecrets(input: string | null | undefined): boolean {
		...
		const allPatterns = [...Object.values(SECRET_PATTERNS), ...this.customPatterns];
	 ...
	 */
	redactObject(obj: Record<string, any> | null | undefined, placeholder = "***REDACTED***"): Record<string, any> {
		...
		for (const [key, value] of Object.entries(obj)) {
			if (typeof value === "string") {
				// Redact string values directly or if key is suspicious
				...
				// Recursively process nested objects, even if key looks suspicious
...
export function redactObject(obj: Record<string, any> | null | undefined, placeholder?: string): Record<string, any> {
